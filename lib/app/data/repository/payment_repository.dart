...
class PaymentRepositoryImplements extends PaymentRepository {
  ...
  @override
  Future<DataState<CreatePaymentResponse>> createPayment(
    int reservationId,
    int paymentMethodId,
    int amount,
  ) async {
  ...
  @override
  Future<DataState<UploadPaymentResponse>> uploadPaymentProof(
    int reservationId,
    List<int> bytes,
    String fileName,
  ) async {
    try {
      final token = await SharedPreferencesHelper.getString(PREF_AUTH);
      final streamedResponse = await _apiService.uploadProofment(
        reservationId,
        fileName,
        bytes,
        token,
      ...
      if (statusCode >= 200 && statusCode < 300) {
        final upload = UploadPaymentResponse.fromJson(responseJson);
        return DataState.success(upload);
      } else {
        print(message);
        return DataState.failed(message);
      }
